...
/*
 * Example test sequence for GPIO validation:
 * - Configure one pin as input (button/switch)
 * - Configure two pins as outputs (LEDs)
 * - Read the input pin
 * - Write output pins
 * - Toggle one output pin
 */
void GPIO_TestExample(void)
{
    /* Configure LED pins as outputs */
    GPIO_SetPinDirection(GPIO_PORTB, GPIO_PIN0, GPIO_OUTPUT);
    GPIO_SetPinDirection(GPIO_PORTB, GPIO_PIN1, GPIO_OUTPUT);

    /* Configure a button/switch pin as input */
    GPIO_SetPinDirection(GPIO_PORTD, GPIO_PIN2, GPIO_INPUT);

    /* Output test: set LED ON */
    GPIO_SetPinValue(GPIO_PORTB, GPIO_PIN0, 1u);

    /* Output test: set LED OFF */
    GPIO_SetPinValue(GPIO_PORTB, GPIO_PIN0, 0u);

    /* Toggle test */
    GPIO_PinToggle(GPIO_PORTB, GPIO_PIN1);

    /* Input test: read the switch/button state */
    if (GPIO_GetPinStatus(GPIO_PORTD, GPIO_PIN2) == PIN_HIGH)
    {
        GPIO_SetPinValue(GPIO_PORTB, GPIO_PIN0, 1u); /* LED ON if button is pressed */
    }
    else
    {
        GPIO_SetPinValue(GPIO_PORTB, GPIO_PIN0, 0u); /* LED OFF otherwise */
    }
}
/*************************************************************************************************************** */